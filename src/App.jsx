...
import { 
  ...
  Map, Shield, AlertTriangle, LocateFixed, Loader2 
} from 'lucide-react';
      ...
      <div className="relative w-[390px] h-[844px] bg-white rounded-[55px] border-[8px] border-slate-800 shadow-2xl flex flex-col overflow-hidden ring-1 ring-black/5">
        ...
        <div className="flex-1 relative flex flex-col pt-12 overflow-hidden bg-white">
          ...
          <div className="absolute bottom-0 left-0 w-full bg-white/80 backdrop-blur-xl border-t border-slate-100 px-8 pt-3 pb-8 flex justify-between items-center z-[90]">
            ...
            <NavBtn icon={<History size={22} />} active={activeTab === 'history'} onClick={() => setActiveTab('history')} />
            <NavBtn icon={<BarChart3 size={22} />} active={activeTab === 'reports'} onClick={() => setActiveTab('reports')} />
            <NavBtn 
...
// DASHBOARD VIEW
// ----------------------------------------------------------------------
function DashboardView({ userName, fuelPercentage, currentFuelLiters, reminders, onSelectModule }) {
  return (
    <div className="px-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-2 gap-3 mt-4">
        ...
        <div className="col-span-2 bg-white rounded-[28px] p-5 border border-slate-50 shadow-sm relative overflow-hidden h-36 flex flex-col justify-between">
           <div className="flex justify-between items-start relative z-10">
              <div className="w-10 h-10 bg-blue-50 rounded-xl flex items-center justify-center text-blue-500"><MapPin size={18} /></div>
              <span className="bg-blue-500 text-white text-[9px] font-bold px-2 py-1 rounded-full">Đang ghi...</span>
           </div>
           <div className="relative z-10"><p className="font-bold text-slate-800">Nhật ký di chuyển</p><p className="text-[10px] text-slate-400">12.4 km • Q1 ➔ Q7, TP.HCM</p></div>
           <svg className="absolute inset-0 w-full h-full opacity-10" viewBox="0 0 400 200"><path d="M0,150 Q100,150 150,100 T300,80" fill="none" stroke="currentColor" strokeWidth="10" className="text-blue-500"/></svg>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="p-6 pt-6 animate-in slide-in-from-right-10 duration-500 h-full">
      <div className="mb-6">
         ...
         <p className="text-slate-400 text-[12px] font-medium mt-1">Tổng quan các hạng mục lớn</p>
      </div>

      <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-6 -mx-6 px-6">
      ...
      <div className="flex justify-between items-end mb-4 mt-2">
         <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">Lịch sử chi tiết</p>
      </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-slate-800 text-sm truncate">
                    {r.type === 'fuel' ? `Đổ dầu (+${r.liters?.toFixed(1)}L)` : r.title}
                  </p>
                  <div className="flex items-center gap-1.5 mt-0.5">
                     ...
                     {r.location && (
                       <>
                        ...
                        <span className="text-[9px] font-bold text-blue-500 truncate max-w-[120px]">{r.location}</span>
                       </>
                     )}
                  </div>
                </div>
...
function ReportsView({ records }) {
  ...
  return (
    <div className="p-6 pt-6 animate-in slide-in-from-right-10 duration-500 h-full">
      <div className="mb-6">
         <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">Analytics</p>
         <h2 className="text-3xl font-black text-slate-800 tracking-tighter">Thống kê</h2>
      </div>

      <div className="mb-6">
        <div className="relative inline-block w-auto bg-white rounded-xl shadow-sm border border-slate-50">
          <div className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-800 pointer-events-none z-10">
          ...
          >
            {availableMonths.map(m => (
               <option key={m} value={m} className="font-medium text-base">Tháng {m}</option>
            ))}
          </select>
          <div className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-800 pointer-events-none z-10">
                ...
                categories.map(cat => {
                  ...
                  return (
                    <div key={cat.id} className="flex-1 flex flex-col items-center gap-1.5 relative group h-full justify-end z-10">
                      <span className={`text-[10px] font-bold transition-all ${value > 0 ? 'text-slate-600' : 'text-transparent'}`}>
                      ...
                      >
                         {value > 0 && (
                           <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] font-bold py-1 px-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap shadow-lg">
                              {new Intl.NumberFormat('vi-VN').format(value)} đ
                           </div>
                         )}
                      </div>
                      <span className="text-[9px] font-bold text-slate-400 mt-2 text-center leading-tight line-clamp-2 px-1">
                         {cat.label}
                      </span>
                    </div>
                  );
    ...
    <div className="p-6 pt-6 animate-in slide-in-from-right-10 duration-500 h-full">
      ...
      >
         ...
         <div className="flex-1">
            ...
            <div 
              ...
              onClick={(e) => { e.stopPropagation(); onEdit('vehicle_details'); }}
            >
      ...
      <div className="space-y-6 pb-8">
         <SettingsGroup title="Cấu hình xe">
            <button 
              onClick={() => onEdit('vehicle_details')} 
              className="w-full flex items-center justify-between p-4 px-5 border-b border-slate-50 active:bg-slate-50 transition-colors text-left"
            >
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     <span className="font-bold text-[14px] text-slate-800 block leading-tight">Thông tin xe & Giấy tờ</span>
                     <span className="font-medium text-[11px] text-slate-400 mt-0.5 block">Bảo hiểm, đăng kiểm...</span>
                  </div>
               </div>
...
function SettingsGroup({ title, children }) {
  return (
    <div>
       ...
       <div className="bg-white rounded-[28px] border border-slate-50 shadow-sm overflow-hidden">
          {children}
       </div>
    </div>
  );
...
function SettingsItem({ icon, label, value, hasToggle, defaultChecked, color = "text-slate-500", hideArrow = false, onClick }) {
   ...
   return (
      <button onClick={onClick} className="w-full flex items-center justify-between p-4 px-5 border-b border-slate-50 last:border-0 active:bg-slate-50 transition-colors">
         <div className="flex items-center gap-4">
            <div className={`${color} bg-slate-50 p-2.5 rounded-xl`}>
               {icon}
            </div>
         ...
         <div className="flex items-center gap-3">
            ...
            {hasToggle && (
               <div
                 ...
                 className={`relative w-11 h-6 rounded-full p-1 transition-colors duration-300 ${checked ? 'bg-blue-500' : 'bg-slate-200'}`}
               >
    ...
    <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm z-[200] flex items-end animate-in fade-in duration-300">
      <div className="w-full bg-white rounded-t-[40px] p-8 pb-12 shadow-2xl animate-in slide-in-from-bottom-full duration-500 max-h-[90vh] overflow-y-auto scrollbar-hide flex flex-col">
        ...
        <div className="space-y-6 flex-1">
          ...
          <DocumentSection title="Phí đường bộ" icon={<Map size={18} className="text-amber-500" />} data={roadFee} isDate onChangeNote={v => setRoadFee(p => ({...p, note: v}))} onChangeDate={v => setRoadFee(p => ({...p, expiryDate: v}))} onFileChange={e => handleFileChange(setRoadFee, e)} onRemove={() => setRoadFee(p => ({...p, image: null}))} />
